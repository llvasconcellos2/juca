...
- `components/SceneView.tsx` — renderiza o `<h2 tabIndex={-1}>` que recebe o foco e, opcionalmente, a imagem da cena (via `components/SceneImage.tsx`) logo **abaixo do label e acima do texto**, seguida do texto da cena, nessa ordem de leitura.
...
- `components/NarrationButton.tsx` — narração por voz via Web Speech API (`speechSynthesis`), em pt-BR, com controles Reiniciar / Ouvir-Pausar / velocidade (0,75×–3×) lado a lado; o `rate` (multiplicador) mora no `StoryEngine`, não neste componente, para persistir entre trocas de cena. Play alterna para Pausar/Continuar via `speechSynthesis.pause()/resume()` (não cancela mais). Trocar a velocidade no meio da fala não reinicia do zero: a última posição ouvida (via `onboundary`) é usada para recomeçar dali, na nova velocidade — cai de volta ao início do segmento em navegadores sem `onboundary` (ex.: Safari). No mount, um efeito **aquece o motor de TTS** chamando `speechSynthesis.getVoices()` cedo (e escutando `voiceschanged`) para evitar o atraso de alguns segundos que o Chrome/Android tem na primeira fala de uma sessão; o mesmo efeito resolve, por melhor esforço, a **voz pt-BR** a usar (`pickNarrationVoice`, prioriza `pt-BR` sobre `pt-PT` e uma lista de nomes conhecidos, para ficar consistente no mesmo aparelho — a API não expõe gênero de voz, então não há garantia de voz masculina em todo aparelho). Além disso, `speechSynthesis.cancel()` só é chamado antes de um novo `speak()` quando já há algo tocando/pendente — chamá-lo sem necessidade é o gatilho conhecido do atraso de vários segundos no motor de TTS do Chrome/Android.
...
- `app/diagnostico-vozes/page.tsx` — **ferramenta permanente de diagnóstico**, fora do fluxo do jogo e não linkada da navegação principal (só acessível via URL direta). Lista todas as vozes que `speechSynthesis.getVoices()` enxerga no aparelho/navegador atual (nome, idioma, se é a padrão do sistema), com botão para testar cada uma em voz alta e botão para copiar o nome exato — usada para descobrir, em aparelhos reais, o nome de uma voz pt-BR de melhor qualidade (ex.: uma voz "de rede" baixada manualmente nos Ajustes do Android) a ser adicionada em `PREFERRED_VOICE_NAMES` (`components/NarrationButton.tsx`). Mantida no projeto (não é para apagar) para servir de diagnóstico sempre que aparecer um novo caso de voz ruim em algum aparelho.
...
- **Imagem de cena (`image`/`imageAlt`/`imagePrompt`/`imageType`), opcional e retrocompatível** — um nó sem `image` renderiza só texto, exatamente como antes:
  ...
  - A imagem aparece **entre o label e o texto** na ordem de leitura/DOM e nunca entra na ordem de tabulação (é `<img>`, não focável — o foco na troca de cena continua indo só para o heading, nunca para a imagem).
...
- **PLANEJADO — ainda não implementado** (a menos que o código mostre o contrário):
  - Narração por voz de **IA/TTS dedicada** (hoje só há a Web Speech API nativa do navegador). A escolha de voz atual (`pickNarrationVoice` em `NarrationButton.tsx`) é **melhor esforço**: prioriza pt-BR e nomes conhecidos, mas cada aparelho só oferece as vozes que tem instaladas e a Web Speech API não expõe gênero — não garante a mesma voz (nem o mesmo gênero) em todo celular. Padronização garantida de voz só vem com esse item (TTS de nuvem ou áudio pré-gravado).
