...
- `app/diagnostico-vozes/page.tsx` — **ferramenta temporária de diagnóstico**, fora do fluxo do jogo e não linkada da navegação principal. Lista todas as vozes que `speechSynthesis.getVoices()` enxerga no aparelho/navegador atual (nome, idioma, se é a padrão do sistema), com botão para testar cada uma em voz alta e botão para copiar o nome exato — usada para descobrir, em aparelhos reais, o nome de uma voz pt-BR de melhor qualidade (ex.: uma voz "de rede" baixada manualmente nos Ajustes do Android) a ser adicionada em `PREFERRED_VOICE_NAMES` (`components/NarrationButton.tsx`). Remover depois que não for mais necessária.
